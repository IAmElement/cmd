...
// ---------------------------------------------------------------------------
/// <summary>
/// Initializes a new instance of the CUDPSocket class.
/// </summary>
/// <param name="address">Hostname/IP address to connect to.</param>
/// <param name="port">Port number.</param>
CUDPSocket::CUDPSocket(const std::string& address, uint32_t port) :
    m_address(address),
    m_port(port),
    m_fd(-1)
{
...
/// </summary>
/// <param name="port">Port number.</param>
CUDPSocket::CUDPSocket(uint32_t port) :
    m_address(),
    m_port(port),
    m_fd(-1)
{
...
/// <summary>
/// Finalizes a instance of the CUDPSocket class.
/// </summary>
...
bool CUDPSocket::open()
{
    ...
    if (m_fd < 0) {
#if defined(_WIN32) || defined(_WIN64)
        ::fprintf(stderr, "Cannot create the UDP socket, err: %lu", ::GetLastError());
#else
        ::fprintf(stderr, "Cannot create the UDP socket, err: %d", errno);
#endif
        return false;
    }
    ...
    if (m_port > 0U) {
        ...
        if (!m_address.empty()) {
            ...
            if (addr.sin_addr.s_addr == INADDR_NONE) {
                ::fprintf(stderr, "The local address is invalid - %s", m_address.c_str());
                return false;
            }
        }
...
#if defined(_WIN32) || defined(_WIN64)
            ::fprintf(stderr, "Cannot set the UDP socket option, err: %lu", ::GetLastError());
#else
            ::fprintf(stderr, "Cannot set the UDP socket option, err: %d", errno);
#endif
            return false;
        }
...
#if defined(_WIN32) || defined(_WIN64)
            ::fprintf(stderr, "Cannot bind the UDP address, err: %lu", ::GetLastError());
#else
            ::fprintf(stderr, "Cannot bind the UDP address, err: %d", errno);
#endif
            return false;
        }
    }
...
int CUDPSocket::read(uint8_t * buffer, uint32_t length, in_addr & address, uint32_t & port)
{
    assert(buffer != NULL);
    assert(length > 0U);
    ...
    if (ret < 0) {
#if defined(_WIN32) || defined(_WIN64)
        ::fprintf(stderr, "Error returned from UDP select, err: %lu", ::GetLastError());
#else
        ::fprintf(stderr, "Error returned from UDP select, err: %d", errno);
#endif
        return -1;
    }
    ...
    if (len <= 0) {
#if defined(_WIN32) || defined(_WIN64)
        ::fprintf(stderr, "Error returned from recvfrom, err: %lu", ::GetLastError());
#else
        ::fprintf(stderr, "Error returned from recvfrom, err: %d", errno);
#endif
        return -1;
    }
...
bool CUDPSocket::write(const uint8_t * buffer, uint32_t length, const in_addr & address, uint32_t port)
{
    assert(buffer != NULL);
    assert(length > 0U);
    ...
    if (ret < 0) {
#if defined(_WIN32) || defined(_WIN64)
        ::fprintf(stderr, "Error returned from sendto, err: %lu", ::GetLastError());
#else
        ::fprintf(stderr, "Error returned from sendto, err: %d", errno);
#endif
        return false;
    }
